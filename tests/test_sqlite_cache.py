...
class SqliteCacheTests(unittest.TestCase):
    def test_backfills_when_sqlite_file_has_no_candles(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            market = CachedMarketClient(upstream, store, backfill_limit=5, clock=lambda: 5_400)

            candles = market.get_candles("BTC-USDT-SWAP", "15m", 5)

            self.assertEqual(5, len(candles))
            self.assertEqual([5], [request[1] for request in upstream.candle_requests])
    ...
    def test_fetches_all_missing_gap_candles_after_latest_local_candle(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            self.assertEqual([4], [request[1] for request in upstream.candle_requests])
    ...
    def test_fetches_only_latest_candle_when_local_is_previous_candle(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            ...
            market = CachedMarketClient(upstream, store, backfill_limit=5, clock=lambda: 3_600)

            candles = market.get_candles("BTC-USDT-SWAP", "15m", 3)

            self.assertEqual([2], [request[1] for request in upstream.candle_requests])
    ...
    def test_skips_upstream_candle_call_when_local_cache_is_current(self) -> None:
        with tempfile.TemporaryDirectory() as directory:
            store = SqliteCandleStore(str(Path(directory) / "candles.sqlite3"))
            store.upsert_candles(
                "BTC-USDT-SWAP",
                "15m",
                [candle(900_000), candle(1_800_000), candle(2_700_000), candle(3_600_000), candle(4_500_000)],
            )
            upstream = FakeUpstream({})
            market = CachedMarketClient(upstream, store, backfill_limit=5, clock=lambda: 5_400)
            ...
            self.assertEqual([], upstream.candle_requests)
            self.assertEqual([2_700_000, 3_600_000, 4_500_000], [item.ts for item in candles])
